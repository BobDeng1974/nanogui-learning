...
void PacketQueue::put(AVPacket *pkt)
{
    ...
    if (pkt->data != flush_pkt.data && pkt1->pkt.buf == NULL) {
...
int64_t VideoState::OgreResource_Seek(void *user_data, int64_t offset,
                                      int whence)
{
...
{
    if ((*this->video_st)->codec->width != 0
            && (*this->video_st)->codec->height != 0) {
        /*
        if (mTexture.isNull())
        {
            static int i = 0;
        ...
        */
        ...
        mTexture->setPixels(&vp->data[0], (*this->video_st)->codec->width,
                            (*this->video_st)->codec->height);
        //mTexture->bind();
    }
    mTexture->draw();
}
    ...
    if (this->av_sync_type == AV_SYNC_VIDEO_MASTER) {
        ...
        for (; i < this->pictq_size - 1; ++i) {
            if (this->pictq[pictq_rindex].pts + threshold <= this->get_master_clock())
                this->pictq_rindex = (this->pictq_rindex + 1) %
                                     VIDEO_PICTURE_QUEUE_SIZE; // not enough time to show this picture
            else
                break;
        }
...
int VideoState::queue_picture(AVFrame *pFrame, double pts)
{
    ...
    vp->pts = pts;
    vp->data.resize((*this->video_st)->codec->width *
                    (*this->video_st)->codec->height * 4);
...
void VideoState::video_thread_loop(VideoState *self)
{
    ...
    avpicture_alloc((AVPicture *)self->rgbaFrame, AV_PIX_FMT_RGBA,
                    (*self->video_st)->codec->width, (*self->video_st)->codec->height);
    ...
    while (self->videoq.get(packet, self) >= 0) {
        ...
        // Decode video frame
        if (avcodec_decode_video2((*self->video_st)->codec, pFrame, &frameFinished,
                                  packet) < 0)
    ...
    try {
        ...
        while (!self->mQuit) {
            if (self->mSeekRequested) {
                ...
                if (streamIndex >= 0)
                    timestamp = av_rescale_q(seek_target, avTimeBaseQ,
                                             self->format_ctx->streams[streamIndex]->time_base);
                ...
                if (av_seek_frame(self->format_ctx, streamIndex, timestamp,
            ...
            if (self->video_st
                    && packet->stream_index == self->video_st - pFormatCtx->streams)
                self->videoq.put(packet);
            else if (self->audio_st
                     && packet->stream_index == self->audio_st - pFormatCtx->streams)
...
int VideoState::stream_open(int stream_index, AVFormatContext *pFormatCtx)
{
    ...
    if (stream_index < 0
            || stream_index >= static_cast<int>(pFormatCtx->nb_streams))
        return -1;
    ...
    switch (codecCtx->codec_type) {
        case AVMEDIA_TYPE_AUDIO:
            ...
            if (!mAudioFactory) {
                std::cerr << "No audio factory registered, can not play audio stream" <<
                          std::endl;
            ...
            if (!mAudioDecoder.get()) {
                std::cerr << "Failed to create audio decoder, can not play audio stream" <<
                          std::endl;
                avcodec_close((*this->audio_st)->codec);
                this->audio_st = NULL;
                return -1;
...
void VideoState::init(const std::string &resourceName)
{
...
#if 0
    this->stream = Ogre::ResourceGroupManager::getSingleton().openResource(
                       resourceName);
    if (this->stream.isNull())
        throw std::runtime_error("Failed to open video resource");
#else
    this->stream = Streams::FileStreamPtr(new Streams::FileStream(
                                              resourceName));
    if (! this->stream->good())
        throw std::runtime_error("Failed to open video resource");
#endif

    AVIOContext *ioCtx = avio_alloc_context(NULL, 0, 0, this, OgreResource_Read,
                                            OgreResource_Write, OgreResource_Seek);
    ...
    ///
    if (!this->format_ctx
            || avformat_open_input(&this->format_ctx, resourceName.c_str(), NULL, NULL)) {
    ...
    for (i = 0; i < this->format_ctx->nb_streams; i++) {
        if (this->format_ctx->streams[i]->codec->codec_type == AVMEDIA_TYPE_VIDEO
                && video_index < 0)
            video_index = i;
        if (this->format_ctx->streams[i]->codec->codec_type == AVMEDIA_TYPE_AUDIO
                && audio_index < 0)
            audio_index = i;
    }
