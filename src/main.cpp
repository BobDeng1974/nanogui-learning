...
{
public:
    ExampleApplication() : nanogui::Screen(Eigen::Vector2i(800, 600),
                                               "NanoGUI Test")
    {
        ...
        auto textbox = new TextBox(window, "Text box");
        ...
        [this, imageView](const Vector2i & index) -> pair<string, Color> {
            ...
            for (int i = 0; i != 4; ++i)
            {
                auto &channelData = imageData[4 * index.y() * textureSize.x() + 4 * index.x() +
                i];
    ...
    virtual void drawContents()
    {
        ...
        Matrix4f mvp;
        mvp.setIdentity();
        mvp.topLeftCorner<3, 3>() = Matrix3f(Eigen::AngleAxisf((float) glfwGetTime(),
                                                               Vector3f::UnitZ())) * 0.25f;
...
int main(int /* argc */, char ** /* argv */)
{
    try {
        ...
        std::string error_msg = std::string("Caught a fatal error: ") + std::string(
                                    e.what());
#if defined(_WIN32)
